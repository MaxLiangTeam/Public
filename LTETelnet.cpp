...
bool CLTETelnet::Login(CString strHost,int nPort,CString strAccount,CString strPassword,int nMode/* = 0*/)
{
	...
	try
	{
		...
		{
			CString strLog;
			strLog.Format("[%s][CSTelnet::Login] 登陆失败退出程序\n",m_strThreadName);
...
{	
	...
	try
	{
		...
		CString strLog;
		strLog.Format("[%s][CSTelnet::Send] 退出接收, 进行解码",m_strThreadName);
		...
		if (strSendMsg.Find("GETCPURATIO") != -1)
		{
			...
			strLog.Format("[%s][CSTelnet::Send] CPU占有率nPos:%d, nLen:%d",m_strThreadName, nPos, nLen);
			g_pLog->WriteLog(strLog,m_strLogFile,LOG_INFO);
		...
		{
			//  发送同步指令, 等待接收异步返回信息
			strLog.Format("[%s][CSTelnet::Send] 发送同步指令, 等待接收异步修改进度信息.",m_strThreadName);
			g_pLog->WriteLog(strLog,m_strLogFile,LOG_INFO);
		...
		strLog.Format("[%s][CSTelnet::Send] 解码完成, 打印输出内容%s",m_strThreadName,m_strNormalText);
		g_pLog->WriteLog(strLog,m_strLogFile,LOG_INFO);
	}
	catch (exception& e)
	{
...
void CLTETelnet::AnalyseMsg( CString strData )
{
	...
	CString strLog;
	strLog.Format("[%s][CSTelnet::AnalyseMsg] 解析指令开始",m_strThreadName);
	g_pLog->WriteLog(strLog,m_strLogFile,LOG_INFO);
	...
	strLog.Format("[%s][CSTelnet::AnalyseMsg] 解析指令结束",m_strThreadName);
	g_pLog->WriteLog(strLog,m_strLogFile,LOG_INFO);
}
